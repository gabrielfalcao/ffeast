...
from ffeast.framework.http import JSONResource, json_response
from ffeast.api.models import User
from ffeast.api.roles import PublicRole
...
def get_authenticated_user(given_role_names=None):
    ...
    token = request.headers.get('X-FFeast-Token', '')
    if not token:
        return None, forbidden('Missing X-FFeast-Token')

    user = User.from_token(token)
    if not user:
        return None, forbidden('Invalid X-FFeast-Token')
