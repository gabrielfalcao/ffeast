...
def get_logger(name='ffeast'): # pragma: no cover
    if settings.TESTING:
        from mock import Mock
        return Mock(name=name)

    if name not in settings.LOGGER_NAMES:
        sys.stderr.write("ffeast.log.get_logger requested a logger name that is not registered in settings.LOGGER_NAMES, you might want to fix it somehow: {0}\n".format(name))
